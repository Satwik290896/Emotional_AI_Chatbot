...
def gatherEmotionAccuracy(y_true,y_pred):
...
if __name__ == "__main__":
    ...
    X_train, Y_train = read_csv('TrainDTSil.csv')
    print("nvrevrveirvnrlivcccccccccccccccccccccccccccccccccccccccrvnervrionvrieovoirevgtegrer")
    print(tf.size(X_train))
    X_test, Y_test = read_csv('TestDTSil.csv')
    ...
    for k in range(1):
        ...
        for i in range(len(X_test)):
            ...
            count = count + gatherEmotionAccuracy(Y_test[i],num)
            #if (num != Y_test[i]):
