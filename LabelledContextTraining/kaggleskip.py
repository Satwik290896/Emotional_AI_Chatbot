...
from keras.layers import Dense, Input, Dropout, Add, LSTM, Activation
...
def SentimentAnalysis(input_shape, word_to_vec_map, word_to_index):
    ...
    # careful, the returned output should be a batch of sequences.
    X = LSTM(128, return_sequences=True)(embeddings)

    # Add dropout with a probability of 0.5
    X = Dropout(0.5)(X)
    A = LSTM(128, return_sequences=True)(embeddings)
    A = Dropout(0.5)(A)
    ...
    X = Add()([A,X])
    # Propaga:te X trough another LSTM layer with 128-dimensional hidden state
...
if __name__ == "__main__":
    ...
    X_train, Y_train = read_csv('training.csv')
    X_test, Y_test = read_csv('validation.csv')
